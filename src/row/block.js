...
if (enableCustomTemplate) {
	templates.custom = {
		...
		templateLock: false,
		blocks: [["wp-bootstrap-blocks/column"]]
	};
}
...
registerBlockType("wp-bootstrap-blocks/row", {
	...
	edit({ className, attributes, setAttributes, clientId }) {
		...
		const onTemplateChange = selectedTemplate => {
			...
			const cols = select("core/editor").getBlocksByClientId(clientId)[0]
				.innerBlocks;
			...
			cols.forEach((col, index) => {
				if (
					...
					templates[selectedTemplate].blocks.length > index
				) {
					const newAttributes =
						templates[selectedTemplate].blocks[index][1];
